...
function requestLocationPermission() {
    if (!navigator.geolocation) {
        setGpsStatus(
            "error",
            "GPS မရနိုင်ပါ",
            "ဒီစက်တွင် Location service မထောက်ပံ့ပါ"
        );
        return;
    }

    /* This call intentionally triggers Android/WebView location permission
       when the native APK has location permissions configured. */
...
function startGPS() {
    ...
    setGpsStatus(
        "weak",
        "GPS ရှာနေသည်",
        "Location permission / signal စောင့်နေပါသည်"
    );

    /* First request a single position so Android can show the permission
       dialog before the continuous watch starts. */
